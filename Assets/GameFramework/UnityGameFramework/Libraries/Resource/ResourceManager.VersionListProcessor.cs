    ...
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed class VersionListProcessor
        {
            ...
            public void UpdateVersionList(int versionListLength, int versionListHashCode, int versionListCompressedLength, int versionListCompressedHashCode)
            {
                ...
                //UnityEngine.Debu("下载地址:{0}  \t 下载本地路径：{1}", Utility.Path.GetRemotePath(Path.Combine(m_ResourceManager.m_UpdatePrefixUri, latestVersionListFullNameWithCrc32)), localVersionListFilePath);
                var path = Path.Combine(m_ResourceManager.m_UpdatePrefixUri, latestVersionListFullNameWithCrc32);
                UnityEngine.Debug.Log($"本地路径：{localVersionListFilePath} \t 下载地址:{path}");
                m_DownloadManager.AddDownload(localVersionListFilePath, path, this);
            }
